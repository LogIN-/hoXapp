 ...
 */
public class NanoHTTPD extends Activity
{
	// ==================================================
	// API parts
	// ==================================================
	 ...
	 */
	public NanoHTTPD( int port, File wwwroot, String PHP_LOCATION ) throws IOException
	{
		myTcpPort = port;
		this.myRootDir = wwwroot;
		NanoHTTPD.PHP_LOCATION = PHP_LOCATION;
	...
	public static void main( String[] args )
	{
		...
		try
		{
			new NanoHTTPD( port, wwwroot, PHP_LOCATION );
		}
		catch( IOException ioe )
		{
	...
	private static String PHP_LOCATION;
							   ...
							   boolean allowDirectoryListing )
	{
		...
		if ( res == null && f.isDirectory())
		{
			...
			if ( res == null )
			{
				...
				else if ( new File( f, "index.php" ).exists())
					f = new File( homeDir, uri + "/index.php" );
		...
		try
		{
			if ( res == null )
			{
				...
				if ( mime == null ){
					...
					Log.e("SERVER_BUAHA_NO_MIME ","NULL");
				}else{
					Log.e("SERVER_BUAHA_MIME ", mime);
				}
				...
				else
				{
					...
					if(mime.equals("application/x-php")){
						
						/*
						String php_file;
						String php_output = "";
						
						php_file = (String)f.toString();						
						String myExec = PHP_LOCATION + " " + php_file;		
				        ...
				        try {
				            ...
				            reader.close();
				            process.waitFor();
				            php_output = output.toString();
				        } catch (IOException e) {
				            throw new RuntimeException(e);
				        } catch (InterruptedException e) {
				            throw new RuntimeException(e);
				        }
						// Log.e("PHP_MANIPULATION_1_F ", "hah:" + php_output + "HAH");
						
						res = new Response( HTTP_OK, "text/html", new FileInputStream( f ));
						*/
					...
					}else if (etag.equals(header.getProperty("if-none-match")) && !mime.equals("application/x-php")){
						res = new Response( HTTP_NOTMODIFIED, mime, "");
						Log.e("PHP_MANIPULATION_2_ETAG ", etag);
						Log.e("PHP_MANIPULATION_2_MIME", mime);
					}else{
						res = new Response( HTTP_OK, mime, new FileInputStream( f ));
						res.addHeader( "Content-Length", "" + fileLen);
						res.addHeader( "ETag", etag);
						Log.e("PHP_MANIPULATION_3_ETAG ", etag);
						Log.e("PHP_MANIPULATION_3_MIME ", mime);
					}
				}
			}
		}
